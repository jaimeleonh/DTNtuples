...
void UseTemplateDeltaTimeQ()
{
  ...
  for (auto & chambTag: chambTags) {
    ...
    TString PlotName = "DeltaTimeQ_" + chambTag + ".pdf";
    bool LogY = false;
  ...
  TH1F *myFunction = (TH1F*)file1->Get(("Plot2_" + chambTag).c_str());
   TH1F *myFunction2 = (TH1F*)file1->Get(("hTMSeg_" + chambTag).c_str());
   ...
   //sil  TF1* myFunction = new TF1("myFunction","x *100",minimumX,maximumX);
    ...
    leg->AddEntry(myFunction,"Phase-2 prim. Q>2");
